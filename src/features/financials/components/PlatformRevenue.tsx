...
// money coming into the platform (withdrawals/refunds are outflows)
const MONEY_IN = ['deposit', 'escrow_lock', 'escrow_release', 'milestone_payment', 'platform_fee'];
...
// Most recent N buckets containing data, so the chart is never empty
// just because activity is sparse or old.
function bucketRevenue(rows: RevenueRow[], period: TrendPeriod) {
  ...
  for (const r of rows) {
    if (!MONEY_IN.includes(r.type)) continue;
...
const RevenueTrendChart: React.FC<{ rows: RevenueRow[] }> = ({ rows }) => {
  ...
  const Pill = ({ p, label }: { p: TrendPeriod; label: string }) => (
    <button onClick={() => setPeriod(p)} style={{
      padding: '4px 12px', borderRadius: 20, border: 'none', fontSize: 12,
      fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit',
      background: period === p ? NAVY : 'transparent',
      color: period === p ? '#fff' : SLATE, transition: 'all 0.15s',
    }}>
      {label}
    </button>
  );
    ...
    <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 14, padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 12 }}>
        <div style={{ marginRight: 'auto' }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: NAVY }}>Revenue Trend</div>
          <div style={{ fontSize: 12, color: SLATE, marginTop: 2 }}>
            Money into the platform
            {trendPct != null && (
              <span style={{ color: trendPct >= 0 ? '#16A34A' : '#DC2626', fontWeight: 700 }}>
        ...
        <div style={{ display: 'flex', gap: 2, background: BG, borderRadius: 22, padding: '3px 4px', border: `1px solid ${BORDER}` }}>
          <Pill p="day" label="Daily" /><Pill p="week" label="Weekly" /><Pill p="month" label="Monthly" />
        </div>
      </div>
        ...
        <div style={{ width: '100%', height: 220 }}>
          <ResponsiveContainer>
            <AreaChart data={points} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11, fill: SLATE }} tickLine={false} axisLine={false}
                tickFormatter={(v: number) => (v >= 1000 ? `${Math.round(v / 1000)}K` : String(v))} />
              <Tooltip formatter={(v: any) => fmtAmt(Number(v))} />
              <Area type="monotone" dataKey="revenue" name="Revenue" stroke={ORANGE}
                strokeWidth={2} fill="url(#prTrendFill)" dot={{ r: 3, fill: ORANGE }} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      )}
