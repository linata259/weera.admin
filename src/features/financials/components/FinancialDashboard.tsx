    ...
    <div style={{ position: "relative" }}>
      <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <div style={{ fontSize: 12, color: SLATE, marginTop: 2 }}>Fee revenue earned by platform</div>
        </div>
        <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", alignItems: isMobile ? "flex-start" : "center", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ width: 20, height: 2.5, borderRadius: 2, background: GRN }} />
      ...
      <div style={{ position: "relative" }}>
        <svg viewBox={`0 0 ${VW} ${VH}`} style={{ width: "100%", display: "block", overflow: "visible" }}>
          <defs>
            <linearGradient id="grad-commission" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={GRN} stopOpacity="0.22" /><stop offset="100%" stopColor={GRN} stopOpacity="0" />
            </linearGradient>
          </defs>
          {yTicks.map((tick, i) => (
            <g key={i}>
          ...
          <path d={linePath} fill="none" stroke={GRN} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
          {hoveredIdx !== null && <line x1={xOf(hoveredIdx)} y1={PAD_T} x2={xOf(hoveredIdx)} y2={baseY} stroke="#CBD5E1" strokeWidth="1" strokeDasharray="4 3" />}
          {pts.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r={hoveredIdx === i ? 5 : 3} fill={GRN} style={{ transition: "r 0.1s" }} />)}
          {sliced.map((d, i) => (
            <g key={i}>
              <line x1={xOf(i)} y1={baseY} x2={xOf(i)} y2={baseY + 5} stroke="#CBD5E1" strokeWidth="0.8" />
              <text x={xOf(i)} y={baseY + 18} textAnchor="middle" fontSize="9.5" fill="#94A3B8" fontWeight="500" fontFamily="'Inter','Helvetica Neue',sans-serif">
        ...
        {tooltip && hoveredIdx !== null && (
          <div style={{ position: "absolute", left: `clamp(0px, calc(${ttX}% - 70px), calc(100% - 150px))`, top: `calc(${ttY}% - 72px)`, pointerEvents: "none", background: NAVY, color: "#fff", borderRadius: 10, padding: "10px 14px", fontSize: 12, boxShadow: "0 8px 24px rgba(15,23,42,0.2)", minWidth: 140, zIndex: 10 }}>
            <div style={{ fontWeight: 700, fontSize: 11, color: "#94A3B8", marginBottom: 6, textTransform: "uppercase", letterSpacing: 0.6 }}>{tooltip.month}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 4 }}>
